...
public:
    ...
    double EvalPhaseSpline(int idx, double x) const { return fSpePS[idx]->Eval(x); }
    // Setters
...
public:
    ...
    typedef std::vector<std::pair<std::string, int>> ChartType;
...
public:
    ...
    const ChartType& GetChart() const { return fChart; }
    int GetDataSize() const { return fData->GetNBinsX(); }
    void PrintChart() const;
    inline std::pair<double, double> EvalAfterFitByBin(int bin, const double* pars) const
    {
        return {fData->GetX(bin), EvalInBin(bin, pars)};
    }
    inline double EvalAfterFitByX(double x, const double* pars) const { return EvalInBin(x, pars, true); }
